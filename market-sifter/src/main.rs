...
async fn main() {
    ...
    let top_coins_with_futures_per_category =
        get_top_coins_per_category(CoinGeckoClient::default(), BinanceFuturesClient::default())
            .await;

    write_coin_info_to_file(&top_coins_with_futures_per_category);

    let change_summary = compare_coin_lists(previous_output, top_coins_with_futures_per_category);
